 ...
 */
abstract class Studioone_ArCa_Controller_Arca_Abstract extends Mage_Core_Controller_Front_Action
{
    /**
     * @var Studioone_ArCa_Model_Arca_Checkout
     */
    ...
    /**
     * Start Express Checkout by requesting initial token and dispatching customer to ArCa
     */
    public function startAction()
    {
        try {
            ...
            $isBARequested = (bool)$this->getRequest()
                ->getParam(Studioone_ArCa_Model_Arca_Checkout::PAYMENT_INFO_TRANSPORT_BILLING_AGREEMENT);
            if ($customer && $customer->getId()) {
            ...
            $this->_checkout->prepareGiropayUrls(
                Mage::getUrl('arca/success'),
                Mage::getUrl('arca/cancel'),
                Mage::getUrl('arca/success')
            );
    ...
    /**
     * Return from ArCa and dispatch customer to order review page
     */
    public function returnAction()
    {
        try {
            $this->_initCheckout();
            $this->_checkout->returnFromArca($this->_initToken());
    ...
    /**
     * Review order after returning from ArCa
     */
    public function reviewAction()
    {
        try {
            ...
            $this->loadLayout();
            $this->_initLayoutMessages('arca/session');
            $reviewBlock = $this->getLayout()->getBlock('arca.express.review');
    ...
    /**
     * Dispatch customer back to ArCa for editing payment information
     */
    ...
    public function saveShippingMethodAction()
    {
        try {
            ...
            if ($isAjax) {
                $this->loadLayout('arca_Arca_review_details');
    ...
    public function updateShippingMethodsAction()
    {
        try {
            $this->_initCheckout();
            $this->_checkout->prepareOrderReview($this->_initToken());
            $this->loadLayout('arca_Arca_review');
        ...
        try {
            ...
            if ($requiredAgreements) {
                ...
                if (array_diff($requiredAgreements, $postedAgreements)) {
                    Mage::throwException(Mage::helper('arca')->__('Please agree to all the terms and conditions before placing the order.'));
                }
            }
            ...
            // redirect if ArCa specified some URL (for example, to Giropay bank)
    ...
    private function _initCheckout()
    {
        ...
        if (!$quote->hasItems() || $quote->getHasError()) {
            ...
            Mage::throwException(Mage::helper('arca')->__('Unable to initialize Express Checkout.'));
        }
     ...
     * @return Studioone_ArCa_ExpressController|string
     */
    protected function _initToken($setToken = null)
    {
        if (null !== $setToken) {
            if (false === $setToken) {
                ...
                if (!$this->_getSession()->getExpressCheckoutToken()) {
                    Mage::throwException($this->__('ArCa Express Checkout Token does not exist.'));
                }
        ...
        if ($setToken = $this->getRequest()->getParam('token')) {
            if ($setToken !== $this->_getSession()->getExpressCheckoutToken()) {
                Mage::throwException($this->__('Wrong ArCa Express Checkout Token specified.'));
            }
        } else {
    ...
    /**
     * ArCa session instance getter
     *
     * @return Studioone_ArCa_Model_Session
     */
    private function _getSession()
    {
        return Mage::getSingleton('arca/session');
    }

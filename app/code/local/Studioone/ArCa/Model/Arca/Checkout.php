...
class Studioone_ArCa_Model_Arca_Checkout
{
    /**
     * Cache ID prefix for "pal" lookup
     * @var string
     */
    const PAL_CACHE_ID = 'Arca_checkout_pal';
     ...
     * @var string
     */
    const PAYMENT_INFO_TRANSPORT_TOKEN    = 'Arca_checkout_token';
    const PAYMENT_INFO_TRANSPORT_SHIPPING_OVERRIDEN = 'Arca_checkout_shipping_overriden';
    const PAYMENT_INFO_TRANSPORT_SHIPPING_METHOD = 'Arca_checkout_shipping_method';
    const PAYMENT_INFO_TRANSPORT_PAYER_ID = 'Arca_checkout_payer_id';
    const PAYMENT_INFO_TRANSPORT_REDIRECT = 'Arca_checkout_redirect_required';
    const PAYMENT_INFO_TRANSPORT_BILLING_AGREEMENT = 'ec_create_ba';
    ...
    /**
     * Config instance
     * @var Studioone_ArCa_Model_Config
     */
    protected $_config = null;

    /**
     * API instance
     * @var Studioone_ArCa_Model_Api_Nvp
     */
    protected $_api = null;
     ...
     * @var string
     */
    protected $_apiType = 'arca/api_nvp';
     ...
     * @var unknown_type
     */
    protected $_methodType = Studioone_ArCa_Model_Config::METHOD_WPP_EXPRESS;
    ...
    public function __construct($params = array())
    {
        ...
        if (isset($params['config']) && $params['config'] instanceof Studioone_ArCa_Model_Config) {
    ...
    /**
     * Checkout with ArCa image URL getter
     ...
     * @return Studioone_ArCa_Model_Arca_Checkout
     */
    ...
    /**
     * Reserve order ID for specified quote and start checkout on ArCa
     *
    ...
    public function start($returnUrl, $cancelUrl)
    {
        ...
        if (!$this->_quote->getGrandTotal() && !$this->_quote->hasNominalItems()) {
            Mage::throwException(Mage::helper('arca')->__('ArCa does not support processing orders with zero amount. To complete your purchase, proceed to the standard checkout process.'));
        }
        ...
        if ($this->_config->requireBillingAddress == Studioone_ArCa_Model_Config::REQUIRE_BILLING_ADDRESS_ALL) {
            $this->_api->setRequireBillingAddress(1);
        }

        // supress or export shipping address
        if ($this->_quote->getIsVirtual()) {
            if ($this->_config->requireBillingAddress == Studioone_ArCa_Model_Config::REQUIRE_BILLING_ADDRESS_VIRTUAL) {
                $this->_api->setRequireBillingAddress(1);
            }
        ...
        // add line items
        $arcaCart = Mage::getModel('arca/cart', array($this->_quote));
        $this->_api->setarcaCart($arcaCart)
            ->setIsLineItemsEnabled($this->_config->lineItemsEnabled)
        ;

        // add shipping options if needed and line items are available
        if ($this->_config->lineItemsEnabled && $this->_config->transferShippingOptions && $arcaCart->getItems()) {
        ...
        if ($profiles = $this->_quote->prepareRecurringPaymentProfiles()) {
            foreach ($profiles as $profile) {
                $profile->setMethodCode(Studioone_ArCa_Model_Config::METHOD_WPP_EXPRESS);
                if (!$profile->isValid()) {
    ...
    /**
     * Update quote when returned from ArCa
     * rewrite billing address by arca
     * save old billing address for new customer
     * export shipping address in case address absence
     *
     * @param string $token
     */
    public function returnFromArca($token)
    {
        $this->_getApi();
        ...
        Mage::getSingleton('arca/info')->importToPayment($this->_api, $payment);
    ...
    public function prepareOrderReview($token = null)
    {
        ...
        if (!$payment || !$payment->getAdditionalInformation(self::PAYMENT_INFO_TRANSPORT_PAYER_ID)) {
            Mage::throwException(Mage::helper('arca')->__('Payer is not identified.'));
        }
    ...
    /**
     * Place the order and recurring payment profiles when customer returned from arca
    ...
    public function place($token, $shippingMethodCode = null)
    {
        ...
        // commence redirecting to finish payment, if arca requires it
        if ($order->getPayment()->getAdditionalInformation(
                Studioone_ArCa_Model_Arca_Checkout::PAYMENT_INFO_TRANSPORT_REDIRECT
        )) {
            $this->_redirectUrl = $this->_config->getExpressCheckoutCompleteUrl($token);
        }

        switch ($order->getState()) {
            // even after placement arca can disallow to authorize/capture, but will wait until bank transfers money
    ...
    protected function _setBillingAgreementRequest()
    {
        ...
        if (!($this->_config->allow_ba_signup == Studioone_ArCa_Model_Config::EC_BA_SIGNUP_AUTO
    ...
    /**
     * @return Studioone_ArCa_Model_Api_Nvp
     */
    ...
    /**
     * Try to find whether the code provided by ArCa corresponds to any of possible shipping rates
     * This method was created only because ArCa has issues with returning the selected code.
    ...
    protected function _matchShippingMethodCode(Mage_Sales_Model_Quote_Address $address, $selectedCode)
    {
        ...
        foreach ($options as $option) {
            if ($selectedCode === $option['code'] // the proper case as outlined in documentation
                || $selectedCode === $option['name'] // workaround: ArCa may return name instead of the code
                // workaround: ArCa may concatenate code and name, and return it instead of the code:

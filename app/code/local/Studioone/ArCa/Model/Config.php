 ...
 * Works with ArCa-specific system configuration
 */
class Studioone_ArCa_Model_Config
{
    /**
     * ArCa Standard
     * @var string
     */
    const METHOD_WPS         = 'arca_standard';

    /**
     * ArCa Website Payments Pro - Express Checkout
     * @var string
     */
    const METHOD_WPP_EXPRESS = 'arca_express';

    /**
     * ArCa Website Payments Pro - Direct Payments
     * @var string
     */
    const METHOD_WPP_DIRECT  = 'arca_direct';

    /**
     * Direct Payments (Payflow Edition)
     * @var string
     */
    const METHOD_WPP_PE_DIRECT  = 'arcauk_direct';

    /**
     * Express Checkout (Payflow Edition)
     * @var string
     */
    const METHOD_WPP_PE_EXPRESS  = 'arcauk_express';
    ...
    const METHOD_BILLING_AGREEMENT  = 'arca_billing_agreement';
    ...
    /**
     * Default URL for centinel API (ArCa Direct)
     *
     * @var string
     */
    public $centinelDefaultApiUrl = 'https://arca.cardinalcommerce.com/maps/txns.asp';
    ...
    protected $_ecStyleConfigMap = array(
        ...
        'arca_hdrimg' => 'hdrimg',
        'arca_hdrbordercolor' => 'hdrbordercolor',
        'arca_hdrbackcolor'   => 'hdrbackcolor',
        'arca_payflowcolor'   => 'payflowcolor',
    );

    /**
     * Currency codes supported by ArCa methods
     *
     * @var array
     */
    protected $_supportedCurrencyCodes = array('AMD'=>'052');

    /**
     * Merchant country supported by ArCa
     *
     * @var array
     */
    ...
    /**
     * Buyer country supported by ArCa
     *
     * @var array
     */
     ...
     *
     * @var array
     * @link https://cms.arca.com/us/cgi-bin/?cmd=_render-content&content_ID=developer/e_howto_api_ECButtonIntegration#id089QD0O0TX4__id08AH904I0YK
     */
    ...
    /**
     * Return merchant country codes supported by ArCa
     *
     * @return array
     */
    ...
    /**
     * Return buyer country codes supported by ArCa
     *
     * @return array
     */
    ...
    /**
     * Get url that allows to edit checkout details on arca side
     *
     * @param $token
     * @return string
     */
    ...
    /**
     * Get url for additional actions that ArCa may require customer to do after placing the order.
    ...
    {
        return sprintf('https://www.%sarca.com/webscr%s',
     ...
     * ArCa will ignore "pal", if there is no total amount specified
     *
    ...
    public function getExpressCheckoutShortcutImageUrl($localeCode, $orderTotal = null, $pal = null)
    {
        ...
        return sprintf('https://www.arca.com/%s/i/btn/btn_xpressCheckout.gif',
            $this->_getSupportedLocaleCode($localeCode));
    }

    /**
     * Get ArCa "mark" image URL
    ...
    public function getPaymentMarkImageUrl($localeCode, $orderTotal = null, $pal = null, $staticSize = null)
    {
        ...
        return sprintf('https://www.arca.com/%s/i/logo/ArCa_mark_%s.gif',
            $this->_getSupportedLocaleCode($localeCode), $staticSize);
    }

    /**
     * Get "What Is ArCa" localized URL
    ...
    public function getPaymentMarkWhatIsPaypalUrl(Mage_Core_Model_Locale $locale = null)
    {
        ...
        return sprintf('https://www.arca.com/%s/cgi-bin/webscr?cmd=xpt/Marketing/popup/OLCWhatIsArCa-outside',
    ...
    {
        return sprintf('https://www.arca.com/%s/i/bnr/%s_solution_PP%s.gif',
            $this->_getSupportedLocaleCode($localeCode),
    ...
    /**
     * Return supported types for ArCa logo
     *
     * @return array
     */
    ...
    {
        return array(
            'wePrefer_150x60'       => Mage::helper('arca')->__('We prefer ArCa (150 X 60)'),
            'wePrefer_150x40'       => Mage::helper('arca')->__('We prefer ArCa (150 X 40)'),
            'nowAccepting_150x60'   => Mage::helper('arca')->__('Now accepting ArCa (150 X 60)'),
            'nowAccepting_150x40'   => Mage::helper('arca')->__('Now accepting ArCa (150 X 40)'),
            'paymentsBy_150x60'     => Mage::helper('arca')->__('Payments by ArCa (150 X 60)'),
            'paymentsBy_150x40'     => Mage::helper('arca')->__('Payments by ArCa (150 X 40)'),
            'shopNowUsing_150x60'   => Mage::helper('arca')->__('Shop now using (150 X 60)'),
            'shopNowUsing_150x40'   => Mage::helper('arca')->__('Shop now using (150 X 40)'),
        );
    }

    /**
     * Return ArCa logo URL with additional options
     *
    ...
    public function getAdditionalOptionsLogoUrl($localeCode, $type = false)
    {
        ...
        return sprintf('https://www.arcaobjects.com/%s/i/bnr/bnr_%s.gif', $locale, $type);
    }
    ...
    {
        return array(
            self::EC_FLAVOR_DYNAMIC => Mage::helper('arca')->__('Dynamic'),
            self::EC_FLAVOR_STATIC  => Mage::helper('arca')->__('Static'),
        );
    }
    ...
    {
        return array(
            self::EC_BUTTON_TYPE_SHORTCUT => Mage::helper('arca')->__('Shortcut'),
            self::EC_BUTTON_TYPE_MARK     => Mage::helper('arca')->__('Acceptance Mark Image'),
        );
    }
    ...
    public function getPaymentActions()
    {
        $paymentActions = array(
            self::PAYMENT_ACTION_AUTH => Mage::helper('arca')->__('Authorization'),
            self::PAYMENT_ACTION_SALE => Mage::helper('arca')->__('Sale')
        );
        if (!is_null($this->_methodCode) && $this->_methodCode == self::METHOD_WPP_EXPRESS) {
            $paymentActions[self::PAYMENT_ACTION_ORDER] = Mage::helper('arca')->__('Order');
        }
    ...
    {
        return array(
            self::REQUIRE_BILLING_ADDRESS_ALL       => Mage::helper('arca')->__('Yes'),
            self::REQUIRE_BILLING_ADDRESS_NO        => Mage::helper('arca')->__('No'),
            self::REQUIRE_BILLING_ADDRESS_VIRTUAL   => Mage::helper('arca')->__('For Virtual Quotes Only'),
        );
    }

    /**
     * Mapper from ArCa-specific payment actions to Magento payment actions
     *
     * @return string|null
     */
     ...
     * "sole" = "Express Checkout for Auctions" - ArCa allows guest checkout
     * "mark" = "Normal Express Checkout" - ArCa requires to checkout with ArCa buyer account only
     *
     * @return array
     */
    ...
    {
        return array(
            self::EC_SOLUTION_TYPE_SOLE => Mage::helper('arca')->__('Yes'),
            self::EC_SOLUTION_TYPE_MARK => Mage::helper('arca')->__('No'),
        );
    }
    ...
    {
        return array(
            self::EC_BA_SIGNUP_AUTO  => Mage::helper('arca')->__('Auto'),
            self::EC_BA_SIGNUP_ASK   => Mage::helper('arca')->__('Ask Customer'),
            self::EC_BA_SIGNUP_NEVER => Mage::helper('arca')->__('Never')
        );
    }
    ...
    /**
     * Payment data delivery methods getter for ArCa Standard
     *
     * @return array
     */
...
//            self::WPS_TRANSPORT_IPN_PDT  => Mage::helper('adminhtml')->__('Both IPN and PDT'),
    ...
    protected function _getDynamicImageUrl($type, $localeCode, $orderTotal, $pal)
    {
        ...
        return sprintf('https://fpdbs%s.arca.com/dynamicimageweb?%s',
    ...
    /**
     * Map ArCa Standard config fields
     *
     * @param string $fieldName
     * @return string|null
     */
    ...
    /**
     * Map ArCa Express config fields
     *
     * @param string $fieldName
     * @return string|null
     */
    ...
    /**
     * Map ArCa Website Payments Pro common config fields
     *
     * @param string $fieldName
     * @return string|null
     */
    ...
    /**
     * Map ArCa common style config fields
     *
     * @param string $fieldName
     * @return string|null
     */
    protected function _mapGenericStyleFieldset($fieldName)
    {
        switch ($fieldName) {
            ...
            case 'page_style':
            case 'arca_hdrimg':
            case 'arca_hdrbackcolor':
            case 'arca_hdrbordercolor':
            case 'arca_payflowcolor':
                return "arca/style/{$fieldName}";
            default:
                return null;
        }
    }
    ...
    {
        return array(
            '0' => Mage::helper('arca')->__('API Signature'),
            '1' => Mage::helper('arca')->__('API Certificate')
        );
    }
    ...
    public function getApiCertificate()
    {
        ...
        return Mage::getModel('arca/cert')->loadByWebsite($websiteId, false)->getCertPath();
    }
}

 ...
 */
Mage::log(__LINE__, null, __FILE__.'.log');
/** @var $installer Mage_arca_Model_Resource_Setup */
$installer = $this;
...
/**
 * Create table 'arca/settlement_report'
 */
$table = $installer->getConnection()
    ->newTable($installer->getTable('arca/settlement_report'))
    ...
    ->addIndex($installer->getIdxName('arca/settlement_report', array('report_date', 'account_id'), Varien_Db_Adapter_Interface::INDEX_TYPE_UNIQUE),
        array('report_date', 'account_id'), array('type' => Varien_Db_Adapter_Interface::INDEX_TYPE_UNIQUE))
    ->setComment('arca Settlement Report Table');
$installer->getConnection()->createTable($table);

/**
 * Create table 'arca/settlement_report_row'
 */
$table = $installer->getConnection()
    ->newTable($installer->getTable('arca/settlement_report_row'))
    ...
    ->addColumn('arca_reference_id', Varien_Db_Ddl_Table::TYPE_TEXT, 19, array(
        ), 'arca Reference Id')
    ->addColumn('arca_reference_id_type', Varien_Db_Ddl_Table::TYPE_TEXT, 3, array(
        ), 'arca Reference Id Type')
    ...
    ->addIndex($installer->getIdxName('arca/settlement_report_row', array('report_id')),
        array('report_id'))
    ->addForeignKey($installer->getFkName('arca/settlement_report_row', 'report_id', 'arca/settlement_report', 'report_id'),
        'report_id', $installer->getTable('arca/settlement_report'), 'report_id',
        Varien_Db_Ddl_Table::ACTION_CASCADE, Varien_Db_Ddl_Table::ACTION_CASCADE)
    ->setComment('arca Settlement Report Row Table');
$installer->getConnection()->createTable($table);

/**
 * Create table 'arca/cert'
 */
$table = $installer->getConnection()
    ->newTable($installer->getTable('arca/cert'))
    ...
    ->addIndex($installer->getIdxName('arca/cert', array('website_id')),
        array('website_id'))
    ->addForeignKey($installer->getFkName('arca/cert', 'website_id', 'core/website', 'website_id'),
        'website_id', $installer->getTable('core/website'), 'website_id',
        Varien_Db_Ddl_Table::ACTION_CASCADE, Varien_Db_Ddl_Table::ACTION_CASCADE)
    ->setComment('arca Certificate Table');
$installer->getConnection()->createTable($table);

/**
 * Add arca attributes to the:
...
$installer->addAttribute('order', 'arca_ipn_customer_notified', array('type' => 'int', 'visible' => false, 'default' => 0));
